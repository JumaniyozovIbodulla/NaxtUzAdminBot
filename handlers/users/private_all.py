...
async def bot_start_all(message: types.Message, state: FSMContext):
    ...
    await create_lead_1(message.from_user.id, message.from_user.full_name, message.from_user.username, message.from_user.language_code, tarif)
...
async def get_phone(msg: types.Message, state: FSMContext):
    await bot.send_chat_action(msg.chat.id, ChatAction.TYPING)
    await state.set_data({"phone_number": msg.text})
    lang = await get_lang(msg.from_user.id)
...
async def get_phone(msg: types.Message, state: FSMContext):
    await bot.send_chat_action(msg.chat.id, ChatAction.TYPING)
    await state.set_data({"business_type": msg.text})
    lang = await get_lang(msg.from_user.id)
...
async def get_phone(msg: types.Message, state: FSMContext):
    ...
    if msg.content_type == "location":
        ...
        await create_lead_2(msg.from_user.id, name, phone_number, business_type, msg.location.latitude, msg.location.longitude)
        await state.clear()
